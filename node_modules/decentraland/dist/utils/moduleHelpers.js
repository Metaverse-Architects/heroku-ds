...
function buildTypescript({ workingDir, watch, production, silence = false }) {
    ...
    return new Promise((resolve, reject) => {
        ...
        child.stdout.on('data', (data) => {
            if (data.toString().indexOf('The compiler is watching file changes...') !==
                -1) {
                if (!silence)
...
async function getOutdatedEcs(workingDir) {
    const decentralandEcs7Version = await getInstalledVersion(workingDir, '@dcl/sdk');
    ...
    if (decentralandEcs6Version) {
        ...
        if (latestVersion && semver_1.default.lt(decentralandEcs7Version, latestVersion)) {
            return {
                package: '@dcl/sdk',
...
async function isCLIOutdated() {
    ...
    if (cliVersionLatest &&
        cliVersion &&
        semver_1.default.lt(cliVersion, cliVersionLatest)) {
        return true;
    }
    else {
        return false;
