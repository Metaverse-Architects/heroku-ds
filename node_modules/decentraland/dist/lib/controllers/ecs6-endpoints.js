...
function setupEcs6Endpoints(components, router) {
    ...
    router.post('/content/entities', async (ctx) => {
        ...
        const res = await (0, node_fetch_1.default)('https://peer.decentraland.org/content/entities', {
            method: 'post',
            headers,
...
function serveFolders(components, router, baseFolders) {
    ...
    async function pointerRequestHandler(pointers) {
        ...
        const remote = (0, catalystPointers_1.fetchEntityByPointer)('https://peer.decentraland.org', pointers.filter(($) => !$.match(/-?\d+,-?\d+/)));
