...
async function wirePreview(components, watch) {
    ...
    const proxySetupPath = fs_extra_1.default.existsSync(proxySetupPathEcs7)
        ? proxySetupPathEcs7
        : proxySetupPathEcs6;
...
async function bindWatch(components, router, sceneUpdateClients) {
    for (const project of components.dcl.workspace.getAllProjects()) {
        ...
        chokidar_1.default.watch(project.getProjectWorkingDir()).on('all', (_, pathWatch) => {
            // if the updated file is the scene.json#main then skip all drop tests
            if (path_1.default.resolve(pathWatch) !==
                path_1.default.resolve(project.getProjectWorkingDir(), sceneFile.main)) {
