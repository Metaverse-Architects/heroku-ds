...
class Project {
     ...
     */
    async getFiles({ ignoreFiles = '', cache = false } = {}) {
    ...
    async checkCLIandECSCompatibility() {
        ...
        if (ecsVersion === 'ecs6') {
            if (cliPackageJson.minEcsVersion &&
                semver_1.default.lt(ecsPackageJson.version, `${cliPackageJson.minEcsVersion}`)) {
                throw new Error([
        ...
        if (ecsPackageJson.minCliVersion &&
            semver_1.default.lt(cliPackageJson.version, ecsPackageJson.minCliVersion)) {
            throw new Error([
...
async function copySample(projectSample, destWorkingDir) {
    ...
    for (let i = 0; i < files.length; i++) {
        ...
        else if (file === project_1.GITIGNORE_FILE ||
            file === project_1.NPMRC_FILE ||
            file === project_1.ESTLINTRC_FILE) {
